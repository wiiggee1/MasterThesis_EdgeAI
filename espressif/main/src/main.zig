...
export fn app_main() callconv(.C) void {
    ...
    const MatrixType = Matrix(f16, 3, 2);
    const Mat = struct {
        name: c_char,
        rows: c_uint,
        cols: c_uint, 
    ...
    // matrix.print_matrix("Hello Embedded Matrix");
    const c_string: [*c]const u8 = @ptrCast(@typeName(@TypeOf(MatrixType))); 
    const c_string_2: [*c]const u8 = @ptrCast(@typeName(@TypeOf(Mat))); 

    // std.log.info("Matrix type: {s}", .{c_string}); 
    // std.log.info("Matrix type 2:nd attempt: (%u)", .{c_string}); 
    const delay_val = utils.ms_to_tick(500);
    while (true) {
        try utils.Delay(esp_idf.TickType_t).init(delay_val).ms();
        std.log.info("Hi from info log ", .{});
        std.log.warn("Hi from warn log eyysysysysysy", .{});
        std.log.debug("I am debugging this crap!", .{});
        
        // Testing varadic args (...) vs zigs .{}: 

        //NOTE: - Varadic args `(...)`, doesn't work with .{}

        esp_idf.esp_log_write(esp_idf.esp_log_level_get("info"), "DEBUGGING", "Mat type: (%s)\n", c_string_2);
        _ = std.c.printf("Matrix type: (%s)\n", c_string);
        // _ = std.c.printf("Mat type: (%s)\n", .{c_string_2});
        // std.log.info("Matrix type: (%s)", .{c_string}); 
