...
pub const ActivationFunction = enum {
    ...
    //TODO: - Make each activation functions private and add an `apply` fn
    // that would switch over the active enum tag / value.
    ...
    pub fn execute_fn(self: ActivationFunction, comptime T: type, comptime LayerSize: usize, mut_data: []T, derive_flag: bool, alpha: ?T) [LayerSize]T {
        var activation_array: [LayerSize]T = mut_data[0..].*; 
        switch (self) {
   ...
   fn leaky_relu(comptime T: type, mut_data: []T, alpha: T, derive_flag: bool) void {
        ...
        const derive = struct {
            pub fn call(val: T) T {
                if (val >  0) @as(T, 1.0) else alpha_val; 
            }
...
test "Activation Function logic validation" {
    ...
    ActivationSoftmax.execute_fn(f16, N, mut_array[0..], false, null);
