...
pub const builder = @import("model_builder.zig");
pub const loader = @import("dataloader.zig");
...
pub const NNModel = builder.NNModel;
pub const HyperParameters = builder.HyperParameters;
pub const ModelBuffer = builder.ModelBuffer; 
pub const DataManager = builder.DataManager;
...
test "data manager reading data, tokenize logic and add_layer" {
    ...
    const manager = try builder.DataManager(u8).init(alloc, "log_data/log_warn.txt");
    try manager.tokenize();

    const params = builder.HyperParameters{
        .input_size = 200,
        .hidden_layers = 3,
        .optimizer = undefined, 
        .learning_rate = 0.001,
    ...
    var nn = builder.NNModel(f16, 4).init(params);
